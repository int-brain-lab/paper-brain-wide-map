...
def get_target_pLeft(
    ...
    for _ in np.arange(nb_sessions):
        ...
        if model is None:
        ...
        elif model.name == modeldispatcher[expSmoothing_prevAction]:
            ...
            act_sim, stim, side, msub_pseudo_tvec = model.simulate(
                arr_params, stim, side, torch.from_numpy(valid), nb_simul=10, only_perf=False, return_prior=True)
...
#            assert False
        ...
        elif model.name == modeldispatcher[expSmoothing_stimside]:
            arr_params = (model.get_parameters(parameter_type='posterior_mean')[None]
                          if subjModel['model_parameters'] is None
                          else np.array(list(subjModel['model_parameters'].values()))[None]
                          )
            ...
            output = model.evaluate(arr_params[None], return_details=True, act=act, stim=stim, side=side)
    ...
    out = np.histogram(
        target_pLeft,
        bins=(np.arange(-bin_size_kde, 1 + bin_size_kde / 2., bin_size_kde) + bin_size_kde / 2.),
        density=True)
