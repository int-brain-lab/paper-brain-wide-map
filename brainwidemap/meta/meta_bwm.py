...
def grid_per_reg(sharedaxs=True):
...
def scatter_analysis_effects(variable, analysis_pair):
    '''
    ...
    s = pd.read_pickle(Path(meta_pth) / f"{variable}.pkl")
    ...
    # Scatter plot
    scatter = ax.scatter(val1, val2, color=cols, s=20)
    
    # Annotating each point with the region code
    for i, reg in enumerate(regss):
        ax.annotate(' ' + reg, (val1[i], val2[i]), fontsize=8, color=cols[i])
    
    # Set labels
    ax.set_xlabel(f'{analysis_pair[0]} amplitude')
    ax.set_ylabel(f'{analysis_pair[1]} amplitude')
    
    # Calculate and display correlation coefficients
    cors, ps = spearmanr(val1, val2)
    corp, pp = pearsonr(val1, val2)
    
    ax.set_title(f'{variable.capitalize()} - Pearson: {np.round(corp,2)}, p={np.round(pp,3)}; Spearman: {np.round(cors,2)}, p={np.round(ps,3)}')
